...
class AddWindow:
    def __init__(self, main_app):
        ...
        tk.Label(self.win, text="Enter Staff name:").pack(pady=10)
        self.entry_name = tk.Entry(self.win)
        self.entry_name.pack(pady=5)

        tk.Label(self.win, text="Enter Staff phone").pack(pady=10)
        self.entry_phone = tk.Entry(self.win)
        self.entry_phone.pack(pady=10)

        tk.Label(self.win, text="Enter Staff email").pack(pady=10)
        self.entry_email = tk.Entry(self.win)
        self.entry_email.pack(pady=10)

        tk.Label(self.win, text="Select Staff Role").pack(pady=10)
    ...
    def save_staff(self):
        ...
        cursor.execute(
            "INSERT INTO staff (name, phone, email, role) VALUES (?, ?, ?, ?)",
...
class EditWindow:
    ...
    def load_staff_data(self, event=None):
        ...
        cursor = self.main_app.cursor
        cursor.execute("SELECT name, phone, email, role FROM staff WHERE id=?", (staff_id,))
    ...
    def save_changes(self):
        ...
        cursor.execute("""
            ...
            WHERE id=?
        """, (name, phone, email, role, self.current_id))
